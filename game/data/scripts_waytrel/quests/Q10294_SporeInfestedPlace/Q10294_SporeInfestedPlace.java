...
public class Q10294_SporeInfestedPlace extends Quest
{
	...
	// Location
	private static final Location TELEPORT_SEA_OF_SPORES = new Location(61012, 24628, -3876);
	// Items
	...
	private static final int MIN_LEVEL = 40; // 40 defoult
	private static final int MAX_LEVEL = 85; // 40 defoult
	// private addCondCompletedQuest(Q10320_LetsGoToTheCentralSquare.class.getSimpleName(), "32975-01a.html");
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		switch (event)
		{
			...
			{
				qs.setCond(2);
				player.teleToLocation(TELEPORT_SEA_OF_SPORES);
    ...
    public void actionForEachPlayer(Player player, Npc npc, boolean isSummon)
	{
		...
		if ((qs != null) && qs.isCond(2) && player.isInsideRadius3D(npc, Config.ALT_PARTY_RANGE))
		{
			...
			else if (allConditionsMet(player, qs))
			{
	...
	{
		return (qs != null) && qs.isCond(2) && (player.getLevel() > 43) && (qs.getInt(KILL_COUNT_VAR) >= 300);
	}
