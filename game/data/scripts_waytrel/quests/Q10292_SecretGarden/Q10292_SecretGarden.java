...
public class Q10292_SecretGarden extends Quest
{
	...
	// Location
	private static final Location TELEPORT_RAYMOND = new Location(-12869, 121706, -2970);
	private static final Location TELEPORT_TO_SACREET_GARDEN = new Location(112709, 136412, -3556);
	// Items
	...
	private static final int MIN_LEVEL = 30; // defoult 30
	...
	public Q10292_SecretGarden()
	{
		...
		addKillId(HARPY, MEDUSA, WYRM, AMBER_BASILISK, TURAK_BUGBEAR, TURAK_BUGBEAR_WARRIOR);
	...
	public String onAdvEvent(String event, Npc npc, Player player)
	{
		...
		switch (event)
		{
			...
			{
				qs.startQuest();
				player.teleToLocation(TELEPORT_RAYMOND);
			...
			{
				qs.setCond(2, true);
				player.teleToLocation(TELEPORT_TO_SACREET_GARDEN);
			...
			case "30289-05.html":
			{
				if (qs.isStarted())
				{
					...
					// addExpAndSp(player, 3000000, 135000);
    ...
    public void actionForEachPlayer(Player player, Npc npc, boolean isSummon)
	{
		...
		if ((qs != null) && qs.isCond(2) && player.isInsideRadius3D(npc, Config.ALT_PARTY_RANGE))
		{
			...
			else
			{
				...
				giveItems(player, SOE_HIGH_PRIEST_RAYMOND);
				player.sendPacket(new ExShowScreenMessage(NpcStringId.MONSTERS_OF_THE_GORGON_FLOWER_GARDEN_ARE_KILLED_USE_THE_TELEPORT_TO_GET_TO_HIGH_PRIEST_RAYMOND_IN_GLUDIO, 2, 5000));
			}
		}
	}
