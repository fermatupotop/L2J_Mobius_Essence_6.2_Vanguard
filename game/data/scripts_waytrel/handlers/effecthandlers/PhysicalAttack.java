...
public class PhysicalAttack extends AbstractEffect
{
	...
	public void instant(Creature effector, Creature effected, Skill skill, Item item)
	{
		...
		if (defence != -1)
		{
			...
			final double baseMod = (weaponMod *(attack * effector.getLevelMod()) + (effector.getStat().getSTR() * power) + rangedBonus) / defence;
			//defoult damage = baseMod * (hasAbnormalType ? _abnormalDamageMod : 1) * ssmod * critMod * weaponTraitMod * generalTraitMod * weaknessMod * attributeMod * pvpPveMod * randomMod;
			//defoult damage *= effector.getStat().getValue(Stat.PHYSICAL_SKILL_POWER, 1);
			...
			damage *= effector.getStat().getValue(Stat.PHYSICAL_SKILL_POWER, 1.5);
		}
