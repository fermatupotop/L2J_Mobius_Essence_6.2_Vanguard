				...
				for (ExtractableProduct expi : exitems)
				{
					...
					if (Rnd.get(100000) <= expi.getChance())
					{
						...
						if (ItemTable.getInstance().getTemplate(expi.getId()).isStackable() || (createItemAmount == 1))
						{
							...
							player.addAutoPeelItem(newItem.getId(), createItemAmount);
							if ((player.getAnnounceType() >= 5) && (player.getAnnounceType() <= 6))
							{
								Broadcast.toAllOnlinePlayers(new ExItemAnnounce(player, newItem, item, 1));
							}
						}
						else
						{
							while (createItemAmount > 0)
							{
								...
								player.addAutoPeelItem(newItem.getId(), 1);
								if ((player.getAnnounceType() >= 5) && (player.getAnnounceType() <= 6))
								{
									Broadcast.toAllOnlinePlayers(new ExItemAnnounce(player, newItem, item, 1));
								}
